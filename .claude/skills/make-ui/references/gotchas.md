...
## G6. 페이지 라우터 미등록 (페이지 전용)

페이지 파일을 생성해도 라우터에 등록하지 않으면 URL로 접근할 수 없습니다.
구현 완료 후 `src/router/` 또는 `src/App.tsx`에 등록 여부를 반드시 확인하고 안내합니다.
라우터 라이브러리가 아직 없으면 설치 및 설정이 필요함을 안내합니다.
